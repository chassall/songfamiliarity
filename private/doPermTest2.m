...
for i = 1:length(neighbours)
    ...
    for j = 1:length(actualClusterExtents)
        ...
        thisData = squeeze(mean(mean(data(:,[thisI neighbI],actualclusterStartI(j):actualclusterEndI(j)),2),3));
        thisDataM = mean(thisData);
        thisDataSD = std(thisData);
        actualClusterDs(j) = thisDataM/thisDataSD;
    end
    ...
    clusterInfo{i,3} = actualClusterDs(maxI);
    clusterInfo{i,4} = scaledWindow;
    % if i == 27
    %     error('*');
    % end
